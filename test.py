...
from unet import *
import cv2
#from qilu import QiluWSI
#from post_processing import *
#from mask2geojson import *
...
def testOne(im_path, device, net, args, update, tasks):
    ...
    if isKFB:
        ...
        w,h = pil_im.size
    Ori = np.zeros((h//scale+psize,w//scale+psize, 3)).astype(np.uint8)
    M = np.zeros((h//scale+psize,w//scale+psize)).astype(np.uint8)
    Heat = np.zeros((h//scale+psize,w//scale+psize, 3)).astype(np.uint8)
    ...
    for j in range(0,h,psize):
        for i in range(0,w,psize):
            #_start = time.time()
            if isKFB:
                patch = wsi.read_roi_local(i-laid,j-laid,psize+2*laid,psize+2*laid)
            elif isTIF:
                patch,_ = wsi.getRegion(
                        region = dict(left=(i-laid)*mag_scale, top=(j-laid)*mag_scale, width=(psize+2*laid)*mag_scale, height=(psize+2*laid)*mag_scale),
                ...
                patch = patch.resize((psize+2*laid, psize+2*laid), Image.BICUBIC)
           ...
           # print('read patch time:',(time.time()-_start))
            ...
            ori_resize = cv2.resize(ori, (resize,resize), interpolation=cv2.INTER_CUBIC)
            #mask_pred = post_processing(mask_pred)
            mask_pred_resize = cv2.resize(mask_pred, (resize,resize), interpolation=cv2.INTER_NEAREST)
            heat_resize = cv2.resize(heat, (resize,resize), interpolation=cv2.INTER_CUBIC)
            Ori[j//scale:j//scale+resize,i//scale:i//scale+resize,:] = ori_resize
            M[j//scale:j//scale+resize,i//scale:i//scale+resize] = mask_pred_resize
            Heat[j//scale:j//scale+resize,i//scale:i//scale+resize,:] = heat_resize
            count += 1
    ...
    if args.save_mask:
        try:
            ...
            try:
                ...
                print(err)
    if args.save_contour:
        feature = mask2geojson(M[:h//scale, :w//scale]*255, upscale=40//args.mag*scale)
        with open(contourpath, 'w') as f:
            geojson.dump(feature, f, indent=4)
    update(im_path, '已完成', tasks)

def get_args(default=False):
    parser = argparse.ArgumentParser(description="generate results based on trained model",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--model', type=str, help='path of the model file .pth')
    parser.add_argument('--src', type=str, help='one file or one folder which the model will depoly on')
    parser.add_argument('--src-ext', type=str, help='src filename suffix/extension, like .png, .kfb', default='.kfb')
    parser.add_argument('--save-dir', type=str, help='the directory to store the results')
    parser.add_argument('--mark', type=str, help='a string will be included in the filenames of results', default='')
    parser.add_argument('--src-mag', type=int, help='the max magnification of WSI the model', default=40)
    parser.add_argument('--mag', type=int, help='the magnification of kfb WSI the model will read on', default=10)
    parser.add_argument('--psize', type=int, help='the patch size of the model input', default=256)
    parser.add_argument('--laid', type=int, help='a padding length will affect the output of the model we use to reduce block effect', default=8)
    parser.add_argument('--scale', type=int, help='save scale according to the mag argument, if src mag is 10, 1 for 10x, 4 for 2.5x', default=4)
    parser.add_argument('--ncpu', type=int, help='process number for multiprocessing to speed up task, file level, need to consdier gpu memory', default=5)
    parser.add_argument('--threshold', type=float, help='the threshold to determine the positive (p>threshold)', default=0.5)
    parser.add_argument('--save-ori', action='store_true', help='save the scaled WSI')
    parser.add_argument('--save-heat', action='store_true', help='save the heatmap overlapped on the scaled WSI')
    parser.add_argument('--save-mask', action='store_true', help='save the scaled binary mask png')
    parser.add_argument('--save-contour', action='store_true', help='save the contours of mask as json file')
    parser.add_argument('--post', action='store_true', help='enable post processing to finetune mask result')
    if default:
...
if __name__ == '__main__':
    ...
    if target.endswith(src_ext):
        ...
        #multiprocessing.set_start_method('spawn')
        p = Pool(args.ncpu)
        im_paths = [p for p in glob(f'{target}/*{src_ext}') if p.endswith('_mask.png')==False]
        ...
        for im_path in im_paths:
            p.apply_async(testOne, (im_path, device, net, args, update_state, Tasks))
