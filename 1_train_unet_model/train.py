...
def train_net(net,
    ...
    if args.load:
        with tqdm(total=n_val,
                  desc="pre load validation round",
                  unit='img',
                  leave=False) as pbar:
    ...
    for epoch in range(start_epoch, epochs + start_epoch):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}',
            ...
            for batch in train_loader:
                ...
                y_true = torch.flatten(true_masks).to(device=device,
                                                      dtype=torch.long)
                ...
                loss = criterion(masks_pred, true_masks)
                if edge:
                    masks_weight = masks_weight.to(device=device,
                                                   dtype=torch.float32)
        ...
        if save_cp:
            ...
            if val_loss < min_loss:
                min_loss = val_loss
                torch.save(net.to(torch.device('cpu')),
                           dir_checkpoint + f'/min_loss.pth')
                net.to(device)
                logging.info(
                    f'Checkpoint {epoch} as new min val loss model saved !')
            if metric['precision'] > best_precision:
                best_precision = metric['precision']
                torch.save(net.to(torch.device('cpu')),
                           dir_checkpoint + f'/best_precision.pth')
                net.to(device)
                logging.info(
                    f'Checkpoint {epoch} as new best precision model saved !')
            if metric['recall'] > best_recall:
                best_recall = metric['recall']
                torch.save(net.to(torch.device('cpu')),
                           dir_checkpoint + f'/best_recall.pth')
                net.to(device)
                logging.info(
                    f'Checkpoint {epoch} as new best recall model saved !')
            if metric['dice'] > best_dice:
                ...
                net.to(device)
                logging.info(
                    f'Checkpoint {epoch} as new best dice model saved !')
...
if __name__ == '__main__':
    ...
    try:
        ...
        torch.save(
            ...
            f'{dir_checkpoint}/INTERRUPTED.pth')
