...
inline void graycomtx(const uint16 *block, const double *opmtx,
   double *comtx, int graylevels, double *is_valid, int rows, int cols, 
   int mrows, int mcols) 
{
   int i,j,k,l;
	int hmrows = mrows/2;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   if(!mxIsClass(prhs[1],"double"))
		mexErrMsgTxt("OPMTX must be DOUBLE.");
	...
	if(nrhs == 4)
   {
      ...
      if(!mxIsClass(prhs[3],"double"))
         mexErrMsgTxt("IS_VALID must be DOUBLE.");
	   is_valid = (double *) mxGetData(prhs[3]);
   }
   else
   {
      is_valid = new double[rows*cols];
